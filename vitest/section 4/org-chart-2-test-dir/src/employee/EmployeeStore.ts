...
export class EmployeeStore {
  ...
  getTeamMembers(employeeId: string): Employee[] {
    const manager = this.getManager(employeeId);
    if (!manager) {
      return [];
    }
    return this.getManageesOf(manager.id);
  }

  getOrgChart(manager: string) {
    // flatmap requires higher ES level
    return this.getManageesOf(manager).flatMap((employee) => [
      employee,
      ...this.getManageesOf(employee.id),
    ]);
  }
}
